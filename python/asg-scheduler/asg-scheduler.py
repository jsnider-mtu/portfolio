...
def main_page():
    ...
    if request.method == 'POST':
        ...
        if not formEventName:
        ...
        elif datetime.strptime(formDate + ' ' + formTime, "%Y-%m-%d %H:%M") < datetime.now():
            ...
            try:
                ...
                err = e
    data = getEvents()
    return render_template('main_page.html', error=err, idp_dict=metadata_url_for, data=data['ScheduledUpdateGroupActions'], submittedDateTime=submittedDateTime, submittedEventName=submittedEventName)
